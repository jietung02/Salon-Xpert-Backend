...
const getUserData = async (userId, role) => {
    try {
        ...
        if (role === 'customer') {
            const sql = "SELECT CUSTOMER_FULL_NAME AS name, USER_EMAIL AS email, CUSTOMER_GENDER AS gender, TIMESTAMPDIFF(YEAR, CUSTOMER_BIRTHDATE, CURDATE()) AS age, CUSTOMER_CONTACT_NUMBER AS contact FROM CUSTOMER AS c INNER JOIN USER AS u ON c.USER_ID = u.USER_ID";
...
const fetchUserPermissions = async (userId, role) => {
    ...
    try {
        ...
        if (role === 'customer') {
            ...
            const sql = "SELECT PERMISSION.PERMISSION_CATEGORY , PERMISSION.PERMISSION_NAME, PERMISSION.PERMISSION_ROUTE FROM USER INNER JOIN STAFF ON USER.USER_ID = STAFF.USER_ID INNER JOIN ROLEPERMISSION ON STAFF.ROLE_CODE = ROLEPERMISSION.ROLE_CODE INNER JOIN PERMISSION ON ROLEPERMISSION.PERMISSION_ID = PERMISSION.PERMISSION_ID WHERE USER.USER_ID = ? ORDER BY PERMISSION_CATEGORY_ORDER, PERMISSION_ORDER_INDEX";
